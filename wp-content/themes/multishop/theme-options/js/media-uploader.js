(function($) {
	$(document).ready(function() {
		...
		function multishop_add_file(event, selector) {
			...
			frame.on( 'select', function() {
				...
				selector.find('.remove-image, .remove-file').on('click', function() {
					multishop_remove_file( $(this).parents('.section') );
				});
			});
		...
		function multishop_remove_file(selector) {
			...
			selector.find('.upload-button').live('click', function() {
				multishop_add_file(event, $(this).parents('.section'));
			});
		}
